...
class job:
  def __init__(self,
    ...
    self.partitions_per_container = math.floor(
  ...
  def schedule_weighted_max_min(self,
    ...
    while queue_length > 0 and curr_containers > 0:
      ...
      allocation = min(
        ...
        math.floor(job.job_weight * self.containers_at_time[time]))
  ...
  def schedule_max_min(self, time=0):
    ...
    while len(self.ready_queue) > 0 and self.containers_at_time[time] > 0:
      ...
      sorted_queue = sorted(self.ready_queue,
                            key=lambda x: self.all_jobs[
                              x.jobID].containers_allotted)
      ...
      for i in range(len(alljobs)):
        ...
        for eachjob in jobs_at_time:
          ...
          if eachjob_parts[0] not in self.all_jobs:
            self.all_jobs[eachjob_parts[0]] = jobInfo(eachjob_parts[0], time,
                                                      math.ceil(float(
                                                          eachjob_parts[
                                                            1]) * float(
                                                          eachjob_parts[2])),
                                                      float(eachjob_parts[3]))
...
sch1 = scheduler('test_dag.txt')
sch2 = scheduler('test_dag.txt')
sch3 = scheduler('test_dag.txt')
sch4 = scheduler('test_dag.txt')
